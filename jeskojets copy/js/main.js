document.addEventListener("DOMContentLoaded", () => {
    ...
    gsap.timeline({
        scrollTrigger: {
            trigger: ".hero",
            start: "top top",
            end: "+=150%", // Scroll distance for the effect
            pin: true,
            scrub: 1
        }
    })
    // Initial reveal is handled by a separate fromTo or normal set, but since we are pinning, let's keep the delay-based reveal
    // but add the scrubbed dive-in effect:
    .to(".content-wrapper", {
        scale: 10,
        opacity: 0,
        ease: "power2.in"
