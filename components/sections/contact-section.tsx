...
export function ContactSection() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const msg = err instanceof TypeError
        ...
        : '메시지 전송 중 오류가 발생했습니다.'
      setError(msg)
      console.error('[ContactSection] 폼 제출 실패:', err)
    } finally {
      setIsPending(false)
    }
  }
  ...
  return (
    <section id="contact" ref={sectionRef} className="relative py-28 px-4">
      {/* 배경 글로우 */}
      <div className="glow-purple-soft absolute bottom-0 left-1/2 -translate-x-1/2 w-96 h-64 rounded-full blur-3xl pointer-events-none" />
            ...
            <GlassCard intensity="medium" className="p-7">
              {sent ? (
                ...
                >
                  ...
                  <p className="font-display font-semibold text-slate-100">Message sent!</p>
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="flex flex-col gap-1.5">
                      <label htmlFor="name" className="text-xs text-slate-500 font-medium uppercase tracking-wide">Name</label>
                      <input
                        id="name"
                        name="name"
                        type="text"
                    ...
                    <div className="flex flex-col gap-1.5">
                      <label htmlFor="email" className="text-xs text-slate-500 font-medium uppercase tracking-wide">Email</label>
                      <input
                        id="email"
                        name="email"
                        type="email"
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label htmlFor="message" className="text-xs text-slate-500 font-medium uppercase tracking-wide">Message</label>
                    <textarea
                      id="message"
                      name="message"
                      required
                  ...
                  <button
                    type="submit"
                    disabled={isPending}
                    className="inline-flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-violet-600 hover:bg-violet-500 disabled:opacity-50 disabled:cursor-not-allowed text-white font-medium text-sm border border-violet-500/30 transition-all shadow-[0_0_24px_rgba(124,58,237,0.3)] hover:shadow-[0_0_36px_rgba(124,58,237,0.5)]"
                  >
                    <Send size={14} />
